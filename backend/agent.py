...
8. Compare prices across Amazon, Flipkart, and Google Shopping (use search_online_prices)

PRICE COMPARISON:
- When the user asks to find, buy, compare prices, or find the best deal for a specific product, use `search_online_prices` to search across Amazon, Flipkart, and Google Shopping.
- Present results as a clear comparison — highlight the cheapest option, mention the source (website), and provide the direct link.
- Sort results by price and clearly state which website offers the cheapest price.
- If the user wants to proceed with a purchase from an external site, explain they can click the link to buy directly from that retailer.
    ...
    def _execute_tool(self, db: DBSession, session_db_id: int, tool_name: str, args: dict) -> dict:
        ...
        try:
            if tool_name == "search_catalog":
                ...
                return T.search_online_prices(session_db_id, **args)
            else:
